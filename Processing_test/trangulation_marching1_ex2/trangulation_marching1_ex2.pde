...
class implicit3d
{
  ...
  void implicit3dset(PVector p)
  {

    f=p.x*p.x+p.y*p.y+p.z*p.z-4;
    //print(f);
    fgradf = new PVector();
    //fgradf.set(p);
    fgradf.x = 2*p.x;
  ...
  void f_gradf_c1(PVector p, float r, float _x1, float _y1)
  {
  ...
  void f_gradf_sph4(PVector p)
  {

       f= p.x*p.x*p.x*p.x+p.y*p.y*p.y*p.y+p.z*p.z*p.z*p.z - 16;
       fgradf = new PVector();
       fgradf.x= 4*p.x*p.x*p.x;
       fgradf.y= 4*p.y*p.y*p.y;
       fgradf.z= 4*p.z*p.z*p.z;
  }
}
...
int ipj, inp, tfrontnpi,ip11, ip22, ip111, ip222, ipi, nn;
float dw, cw, sw, cdw, sdw;
int np=0;    
int nf= 0;      
int ne= 0;  
int ne0= 0; 
...
void draw()
{
 ...
 for(int i=0; i<tnpmax; i++)
 {
   tpoint[i]= new tpoint_dat();
   tpoint[i].full= false;     
 }
  ...
  x1=-2;
  y1=-5;
  p1 = new PVector(1, 1, 1);
  tpoint[0] = new tpoint_dat();
  tpoint[0].p =new PVector();
  tpoint[0].nv =new PVector();
  tpoint[0].tv1 =new PVector();
  tpoint[0].tv2 =new PVector();
  ...
  tpoint[0].p =tpoint[1].p;
 tpoint[0].nv =tpoint[1].nv;
  tpoint[0].tv1 =tpoint[1].tv1;
  tpoint[0].tv2 =tpoint[1].tv2;
...
void start_triangulation(PVector _p_start, implicit3d f_gradf)
{
  ...
  surface_point_normal_tangentvts(_p_start, f_gradf, p0, nv0, tv10, tv20, 0);
  if (!point_ok(p0))
  {
    n_triang=0;
    return;
...
void surface_point_normal_tangentvts(PVector _p_start, implicit3d f_gradf, PVector _p, 
...
void newton_step(PVector _p_i)
{
  ...
  gradfi = new PVector();
  //f_gradf.f_gradf_c1(_p_i, r, x1, y1);
  //f_gradf.f_gradf_sph4(_p_i);
  f_gradf.implicit3dset(_p_i);
  fi= f_gradf.f;
  gradfi=f_gradf.fgradf;
  print(gradfi, fi);
  ...
  if (cc>eps8)
    ...
    t=0;
    println(cc+" WARNING tri (surface_point...): newton");
  }
...
void triangulation(boolean nearpointtest)
{
  ...
  for (int k=1; k<=tnfr; k++)
  {
    for (int i=1; i<=tfr[k][0]; i++)
    {
  ...
  while ((tnfr>=0)&&(tnf<n_triang)&&(fullcount<tfrontnp))
  {
    ...
    {  
      ...
      if (ipf2>0)
      {
        ...
        delay=0;
        println("\n"+"triangles:"+tnf+" nearpoints detected:"+ipi1+"+++ "+ipi2);
        if (frontnumber==0) //<>//
  ...
  print("*********************************************\n");
  println("triang.: total number of triangles: "+tnf);
  println("triang.: remaining front points: "+tfrontnp);
  println("triang.: remaining fronts: "+ tnfr);
  print("*********************************************\n");
  if (tnf>tnfmax)
    println("triang. warning:tnfmax to small!!!!");
  if (tnp>tnpmax)
    println("triang. warning:tnpmax to small!!!");
  if (tfrontnp>tfrnpmax)
    println("triang. warning: tfrnpmax to small!!!");
  if (tnfr>tnfrmax)
      ...
      if ( abs(p0.x-tpoint[ipi].p.x)<tstepl) {
        if (abs(p0.y-tpoint[ipi].p.y)<tstepl) {
          if (abs(p0.z-tpoint[ipi].p.z)<tstepl) {    
            if (distance3d(tpoint[ipi].p, p0 ) < tstepl) 
            {
              if (!outside(ip, tfrontpt[reduce(_ipf-1)], ipi))  //<>//
              {
                continue label10; //<>//
              }
...
{//determinant of 3 vectors.
  ...
  float determ3d= (v1.x*v2.y*v3.z + v1.y*v2.z*v3.x + v1.z*v2.x*v3.y - v1.z*v2.y*v3.x - v1.x*v2.z*v3.y - v1.y*v2.x*v3.z);
  return determ3d;
  //end determ3d
}
...
{ //<>//
  ...
  for (int i=1; i<=(tfrontnp-_ipf2+1); i++)
  {
...
void complete_point(int _ipf, implicit3d f_gradf)
{
  ...
  pn11=newcoordinates3d(pn1, tpoint[ip].p, tpoint[ip].tv1, tpoint[ip].tv2, tpoint[ip].nv);
  pn11.z=0;
  pn11= normalize3d(pn11);
  pn11=scale3d(tstepl, pn11);
  pn2=tpoint[ip2].p;
  pn22=newcoordinates3d(pn2, tpoint[ip].p, tpoint[ip].tv1, tpoint[ip].tv2, tpoint[ip].nv);
  if (tpoint[ip].achange)
    tpoint[ip].angle=fangle(_ipf);
  ne_rest= floor(tpoint[ip].angle*3/PI)+1;
  dw= tpoint[ip].angle/(ne_rest);
  ...
  float ss = 1.25*tstepl;
  if (((ne_rest==1)&&(dw>0.8)&&(distance3d>ss)))
  {
    ne_rest=2; 
    dw=dw/2;
  }
  ...
  float distance3d_square1 = distance3d(pn1, p0);
  //float distance3d_square1 = distance3d_square(p0, pn1);
  float distance3d_square2 =distance3d(pn2, p0);
  //float distance3d_square2 =distance3d_square(p0, pn2);
  //float tt = 0.2*tstepl_square;
  float tt = 0.5*tstepl;
  if ((dw<3)&&((distance3d_square1<=tt)||(distance3d_square2<=tt)))
  {
    ne_rest=1;
  }
  if (ne_rest==1)
  ...
  } else
  {
    for (i=1; i<=ne_rest-1; i++)
    {
      ...
      float scalarp3d1 = scalarp3d(tpoint[tnp].nv, tpoint[ip].nv);
      if (scalarp3d1<0)
      {
      ...
      if (i==ne_rest-1)  
        new_triangle(tnp+(ne_rest-1), ip2, ip);
      else
