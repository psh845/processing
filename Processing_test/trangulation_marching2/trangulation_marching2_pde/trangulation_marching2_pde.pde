...
int np=0;    int nf= 0;      int ne= 0;  int ne0= 0; int npf=0;
...
void draw()
{
  ...
  strokeWeight(0.05);
   ...
   for(int i=0; i<tnfmax; i++)
  {
     tface[i]= new tface_dat(); 
  }
  for(int i=0; i<tnpmax; i++)
  {
    tpoint[i] = new tpoint_dat();
    tpoint[i].full = false;
    tpoint[i].p = new PVector();
  }
  ...
  while ((tnfr>=0)&&(tnf<n_triang)&&(fullcount<tfrontnp))
  {
    ...
    {  
      ...
      if (ipf2>0)
      {
        ...
        delay=0;
        println("triangles:'.tnf.' nearpoints detected:'.ipi1.'+++ "+ipi2);
        if (frontnumber==0)
