...
void draw()
{
  ...
  //start_triangulation(p_start, f_gradf);
  //println(tnp);
  ...
  tnp=tfrontnp;
  dw=TWO_PI/tfrontnp;
  tstepl=r*TWO_PI/tfrontnp;
  for (int i=1; i<=tfrontnp; i++)
  {
    tpoint[i] = new tpoint_dat();
    p1=put3d(x2+r*cos((i-1)*dw), y2+r*sin((i-1)*dw), 5); 
    surface_point_normal_tangentvts(p1, f_gradf, tpoint[i].p, tpoint[i].nv, tpoint[i].tv1, tpoint[i].tv2, 1, i);
    tpoint[i].full=false;
    tpoint[i].achange=true;
    tfrontpt[i]=i;
  }
  dw=-dw;
  for (int i=1; i<=tfrontnp; i++)
  {
    tpoint[tnp+i] = new tpoint_dat();
    p1=put3d(x2+r*cos((i-1)*dw), y2+r*sin((i-1)*dw), -2);
    surface_point_normal_tangentvts(p1, f_gradf, tpoint[tnp+i].p, tpoint[tnp+i] .nv, tpoint[tnp+i].tv1, tpoint[tnp+i].tv2, 1, tnp+i);
    tpoint[tnp+i].full=false;
    tpoint[tnp+i].achange=true;
    tfr[1][i]=tnp+i;
  }
  tfr[1][0]=tfrontnp;
  tnp=tnp+tnp; //<>//
...
void start_triangulation(PVector p_start, implicit3d f_gradf)
{
  ...
  surface_point_normal_tangentvts(p_start, f_gradf, p0, nv0, tv10, tv20, 0, 0);
  if (!point_ok(p0))
  {
    n_triang=0;
    return;
  ...
  for (i=0; i<=5; i++)
  {
    ...
    tfrontpt[i+1]=tnp;
    tpoint[tnp] = new tpoint_dat();
    surface_point_normal_tangentvts(p1, f_gradf, tpoint[tnp].p, tpoint[tnp].nv, tpoint[tnp].tv1, tpoint[tnp].tv2, 1, tnp);
...
void surface_point_normal_tangentvts(PVector p_start, implicit3d f_gradf, PVector _p, 
  PVector _nv, PVector _tv1, PVector _tv2, int num, int _tnp)
{
  p = new PVector();
  nv = new PVector();
  tv1 = new PVector();
...
void complete_point(int _ipf, implicit3d f_gradf)
{
  ...
  if (((ne_rest==0)&&(dw>0.8)&&(distance3d>ss)))
  {
    ne_rest=1; 
    dw=dw/2;
  }
  ...
  if (((distance3d_square<tt)||(distance3d_square1<tt)))
  {
    ne_rest=0;
  }
  if (ne_rest==0)
  ...
  } else
  {
    for (i=1; i<=ne_rest; i++)
    {
      ...
      surface_point_normal_tangentvts(pc_start, f_gradf, tpoint[tnp].p, tpoint[tnp].nv, tpoint[tnp].tv1, tpoint[tnp].tv2, 1, tnp);
