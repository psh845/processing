...
class implicit3d
{
  ...
  void f_gradf_sph4(PVector p)
  {
    ...
    fgradf.x= 4*p.x*p.x*p.x*p.x;
    fgradf.y= 4*p.y*p.y*p.y*p.y;
    fgradf.z= 4*p.z*p.z*p.z*p.z;
  }
}
...
void complete_point(int _ipf, implicit3d f_gradf)
{
  ...
  tv10= tpoint[ip].tv1;
  tv20 = tpoint[ip].tv2;
  //float distance3d_square = distance3d(pn1, p0);
  float distance3d_square = distance3d_square(p0, pn1);
  //float distance3d_square1 =distance3d(pn2, p0);
