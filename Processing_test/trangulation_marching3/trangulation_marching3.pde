...
class implicit3d
{
  ...
  void f_gradf_tor(PVector p, float a_tor)
  {
    c1= sq(r_tor);
    c2= p.x*p.x+p.y*p.y +p.z*p.z + c1 - sq(a_tor);
    f= c2*c2 - 4*c1*(p.x*p.x+p.y*p.y);
    fgradf = new PVector();
    fgradf.x = 4*p.x*c2 - 8*c1*p.x;
    fgradf.y = 4*p.y*c2 - 8*c1*p.y;
    fgradf.z = 4*p.z*c2;
  }
}
...
void draw()
{
 ...
 //cuttype= 1; xcut= 0;  ycut= 0; rcut_square= sq(r_tor);
  tstepl=0.45;
  xmin=-100;
  xmax=100;
  ymin=-100;
...
void newton_step(PVector _p_i)
{
  ...
  f_gradf.f_gradf_tor(_p_i, a_tor);
  fi= f_gradf.f;
