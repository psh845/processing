...
class implicit3d
{
  ...
  {
    f= sq(p.x-_x1)+sq(p.y-_y1)+sq(p.z)-sq(r);
    fgradf = new PVector();
    fgradf.x = 2*(p.x-_x1);
    fgradf.y = 2*(p.y-_y1);
    fgradf.z = 2*(p.z);
...
void draw()
{
  ...
  x1=0;
  y1=0;
  //p1 = new PVector(0, -3, 1);
  p1= new PVector(0,0,3);
...
void surface_point(PVector _p0)
{
  float delta;
...
label10: 
  for (i=_ipf+3; i<=nn; i++) 
  {
    ...
    if (!tpoint[ipi].full) 
    {
      println(p0.x,tpoint[ipi].p.x,abs(p0.x-tpoint[ipi].p.x));
      if ( abs(p0.x-tpoint[ipi].p.x)<tstepl) {
        println(p0.y,tpoint[ipi].p.y,abs(p0.y-tpoint[ipi].p.y));
        if (abs(p0.y-tpoint[ipi].p.y)<tstepl) {
          println(p0.z,tpoint[ipi].p.z,abs(p0.z-tpoint[ipi].p.z));
          if (abs(p0.z-tpoint[ipi].p.z)<tstepl) {    
            println(distance3d_square(p0, tpoint[ipi].p)); //<>//
            if (distance3d_square(p0, tpoint[ipi].p) < tstepl_square) 
            { //<>//
              println(outside(ip, tfrontpt[reduce(_ipf-1)], ipi));
